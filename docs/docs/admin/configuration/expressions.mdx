...
```ts
function remoteAddrInList(remoteAddr: string, ipList: stringList): bool
```

Test if supplied IP address is in list of CIDR address ranges.

```yaml
# Denies clients using Wget or lftp from the set of addresses in the list
